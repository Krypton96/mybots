...
class DBCommands:
    ...
    CHECK_REFERRALS = "SELECT chat_id FROM users WHERE referral=(SELECT id FROM users WHERE chat_id=$1)"
...
async def register_user(message : types.Message):
    ...
    text += f"""
Сейчас в базе <b>{count_users}</b> человек!

Ваша реферальная ссылка:\n {bot_link}
Проверить рефералов можно по команде: <b>/referrals</b>

Ваш баланс: <b>{balance}</b> монет.
Добавить монет: <b>/add_money</b>
"""
